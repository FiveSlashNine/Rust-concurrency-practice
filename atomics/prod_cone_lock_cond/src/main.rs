use std::thread;
use std::sync::{Arc, Mutex, Condvar, atomic::{Ordering, AtomicI32}};
...
impl Buffer {
    ...
    fn put(&self, data: i32) {
        ...
        while self.counter.load(Ordering::Acquire) == self.size as i32 {
            println!("The buffer is full");
            lock = self.buffer_full.wait(lock).unwrap();
        }
    ...
    fn get(&self) -> i32 {
        ...
        while self.counter.load(Ordering::Acquire) == 0 {
            ...
            lock = self.buffer_empty.wait(lock).unwrap();
        }
        ...
        if self.counter.load(Ordering::Acquire) == self.size as i32 - 1 {
...
fn main() {
    ...
    let producer_delay = 100;
    let consumer_delay = 1;
    let no_prods = 3;
    let no_cons = 2;
